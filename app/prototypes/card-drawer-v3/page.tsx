...
export default function CardDrawerV3Prototype() {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-neutral-900 rounded-2xl overflow-hidden flex flex-col" style={{ height: '85vh' }}>
          ...
          {activeTab === 'tasks' && (
            <div className="px-4 py-3">
              ...
              <div className="space-y-1">
                {tasks.map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-neutral-800/50 group cursor-pointer">
                    <button
                      onClick={() => setTasks(ts => ts.map(t => t.id === task.id ? { ...t, done: !t.done } : t))}
                      className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        task.done ? 'bg-violet-500 border-violet-500' : 'border-neutral-600 hover:border-neutral-400'
                      }`}
                    >
                      {task.done && (
                    ...
                    <span className={`text-sm flex-1 ${task.done ? 'text-neutral-500 line-through' : 'text-neutral-200'}`}>
                      {task.title}
                    </span>
                  </div>
                ))}
        ...
        {/* Bottom area — quick-nav toolbar + input, always visible */}
        <div className="flex-shrink-0 border-t border-neutral-800">
          {/* Quick-nav links — Cursor-style, above the input */}
              ...
              >
                ...
                {incompleteTasks > 0 && (
                  <span className={`inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold rounded-full ${
                    activeTab === 'tasks' ? 'bg-violet-500 text-white' : 'bg-neutral-700 text-neutral-400'
                  }`}>
                    {incompleteTasks}
                  </span>
                )}
