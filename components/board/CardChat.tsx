...
export function CardChat({ card, channelName, channelDescription, tagDefinitions = [], tabBar, composerSlot, hasApps = false, onShowApps }: CardChatProps) {
  ...
  const handleSubmit = async (content: string, type: CardMessageType, imageUrls?: string[], imageSettings?: { aspectRatio: string; quality: string; model?: string; background?: string }) => {
      ...
      {card.summary ? (
        ...
        <div className="flex-shrink-0 px-4 py-2">
          ...
          >
            ...
            {isSummaryLoading ? 'Generating summary...' : 'Generate AI summary'}
          </button>
        </div>
      )}
