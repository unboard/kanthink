...
import type { Card, Task, ID, TagDefinition } from '@/lib/types';
  ...
  const tabTiles = (
    <div className="flex gap-2 px-3 pb-2 overflow-x-auto scrollbar-none">
      {([
        ...
        // "App" — the artifact, not the activity. Building happens in the thread
        // via /build; this tab is where the built thing lives.
        { key: 'playground' as const, label: `App${playgroundVersion > 0 ? ` v${playgroundVersion}` : ''}`, icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /> },
      ]).map((tab) => (
        <button
          key={tab.key}
