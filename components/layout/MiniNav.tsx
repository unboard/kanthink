  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-2 flex-1">
        ...
        {/* Kanwatch — admin-only while it is being tried out */}
        {session?.user?.isAdmin && (
          <NavIconButton
            isActive={pathname.startsWith('/kanwatch')}
            onPointerDown={handleNavigate('/kanwatch')}
            label="Kanwatch"
            icon={
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="8.5" strokeWidth={1.5} />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 7.5V12l3 2" />
              </svg>
            }
          />
        )}
